...
class FacebookGroupify extends core.Construct {
  constructor(scope, id) {
    ...
    const spotifySongMatcher = new lambda.Function(this, 'SpotifySongMatcher', {
      runtime: lambda.Runtime.NODEJS_10_X,
      functionName: `${FacebookGroupify.name}-SpotifySongMatcher`,
      description: 'Searches for song title matches in Spotify',
      code: lambda.Code.fromAsset(path.join(__dirname, APP_DIRECTORY)),
      handler: 'functions/SpotifySongMatcher/index.handler',
      timeout: core.Duration.seconds(300),
    });
    bucket.grantReadWrite(spotifySongMatcher);
    spotifySongMatcher.addEventSource(new lambdaEventSources.S3EventSource(bucket, {
      events: [s3.EventType.OBJECT_CREATED],
      filters: [{ prefix: 'titles/youtube/' }],
    }));
    ssmParams.grantRead(spotifySongMatcher);
